"""
==============================================================
 Causal Discovery & Anomaly Detection (Healthcare - RBF Authentic TS-CAM-UV)
==============================================================
"""
...
DETECTION_THRESHOLD_MULTIPLIER = 0.02
...
def compute_online_rbf_errors(
    ...
    for t in range(max_time):
        for i, var in enumerate(np.unique(indices[1, :])):
            ...
            stack = [
                data[max_delay - el[2] : end_idx - el[2], el[0]] for el in var_indices
            ]
            X = np.column_stack(stack)
            y = data[max_delay:end_idx, var]

            X_scaled = scalers[var].transform(X)

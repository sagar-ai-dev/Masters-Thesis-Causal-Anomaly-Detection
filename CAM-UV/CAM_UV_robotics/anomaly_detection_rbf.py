"""
==============================================================
 Causal Discovery & Anomaly Detection (RBF Authentic TS-CAM-UV)
==============================================================
"""
...
DETECTION_THRESHOLD_MULTIPLIER = 0.05
...
# ================================================================
def fit_normal_rbf_coeffs(normal_data: np.ndarray, causal_matrix: np.ndarray, gamma: float = RBF_GAMMA, n_components: int = RBF_N_COMPONENTS, alpha: float = RIDGE_ALPHA):
    ...
    for var in np.unique(indices[1, :]):
        ...
        rbf_map = RBFSampler(gamma=0.01, n_components=100, random_state=42)
        ...
        rbf_maps[var] = (scaler, rbf_map)

    return fine_coeffs, indices, rbf_maps


def compute_online_rbf_errors(data: np.ndarray, fine_coeffs: dict, causal_matrix: np.ndarray, indices: np.ndarray, rbf_maps: dict, alpha: float = RIDGE_ALPHA):
    from sklearn.linear_model import Ridge
    ...
    for t in range(max_time):
        for i, var in enumerate(np.unique(indices[1, :])):
            ...
            scaler, rbf_map = rbf_maps[var]
            X_scaled = scaler.transform(X)
            X_rbf = rbf_map.transform(X_scaled)
...
def score_anomaly_timeline(err_normal, err_attack, normal_data_len, normal):
    ...
    # Apply a rolling MEDIAN filter to perfectly eliminate mechanical noise
    window_size = max(1, CONSECUTIVE_K * 5)
    smoothed_scores = np.copy(scores)
    for i in range(len(scores)):
        smoothed_scores[i] = np.median(scores[max(0, i-window_size+1):i+1])

    predictions = (smoothed_scores > 1.0).astype(int)
    return predictions, smoothed_scores


def main():
    print(f"\n========== TASK: {TASK.upper()} ==========")
    print(f"RBF Gamma: {RBF_GAMMA}, Components: {RBF_N_COMPONENTS}")
    ...
    fine_coeffs, indices, rbf_maps = fit_normal_rbf_coeffs(normal_data, normal_matrix)
    err_normal, norm_agg_normal = compute_online_rbf_errors(normal_data_full, fine_coeffs, normal_matrix, indices, rbf_maps)
    _ = norm_agg_normal
    ...
    for path, df_attack in zip(attack_paths, attack_dfs):
        ...
        err_attack, norm_agg_attack = compute_online_rbf_errors(attack_data, fine_coeffs, normal_matrix, indices, rbf_maps)

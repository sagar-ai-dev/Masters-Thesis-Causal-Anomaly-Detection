"""
==============================================================
 Causal Discovery & Anomaly Detection (Polynomial-Tigramite - Industrial / TEP)
==============================================================
...
def fit_normal_poly_coeffs(normal_data: np.ndarray, causal_matrix: np.ndarray,
    ...
    Visual:
        X_t = [parents with lags] --> PolynomialFeatures --> Ridge
        -----------------------------------------------------------
    ...
    for var in np.unique(indices[1, :]):
        ...
        var_indices.sort(key=lambda x: x[2])
        max_delay = var_indices[-1][2]
        # Limit parent-lag inputs to keep degree-3 polynomial expansion tractable.
        var_indices = var_indices[:3]
...
def main():
    ...
    # Keep the variable for parity with the linear script's workflow.
    _ = norm_agg_normal

...
DETECTION_THRESHOLD_MULTIPLIER = 0.2
...
def learn_causal_model(normal_csv_path: str, save_path: str):
    ...
    nonconst = [
        ...
        if np.std(normal_data.values[0][:, idx])
        > 0.01 * np.mean(normal_data.values[0][:, idx])
    ]
    nonconst_data = normal_data.values[0][:, nonconst]
    for j in range(np.shape(nonconst_data)[1]):
        nonconst_data[:, j] /= (
            (np.max(nonconst_data[:, j]) - np.min(nonconst_data[:, j]))
            + np.min(nonconst_data[:, j])
        )
...
def fit_normal_rbf_coeffs(
    ...
    Visual:
        lagged parents -> StandardScaler -> RBFSampler -> Ridge
        -------------------------------------------------------
    ...
    for var in err_attack.keys():
        for j in range(err_attack[var].shape[1]):
            ...
            if not normal:
                ...
                indices_error += list(
                    np.where(abs(err_attack[var][normal_data_len:, j]) > thresh)[0]
                )
...
def main():
    ...
    attack_paths = [os.path.join(PREFIX, fname) for fname in ATTACK_FILES]

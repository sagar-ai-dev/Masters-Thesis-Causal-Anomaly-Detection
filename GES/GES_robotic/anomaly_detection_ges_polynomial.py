"""
==============================================================
 Target-Wise GES Robotics Anomaly Detection: Polynomial Degree 3
==============================================================

Purpose
-------
This standalone PCMCI-style script runs the target-wise GES anomaly-detection pipeline with a degree-3 Polynomial downstream regression. It uses the same fixed GES graph, preprocessing, cache behavior, thresholding style, and metric formulas as the Linear and RBF scripts.
...
parent-lag design matrix X and the current target value is y.
    ...
    params  = degree = 3, ridge_alpha = 10.0, threshold_multiplier = 1.5
...
def score_and_predict(err_normal, err_eval, normal_data_len: int, normal: bool, threshold_multiplier: float, total_samples=None):
    ...
    The binary predictions use the same threshold rule as detect_anomalies(...).
    The continuous score is the maximum threshold-normalized drift over all
    target/coefficient traces at each time index.
    """

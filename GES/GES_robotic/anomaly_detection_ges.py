"""
==============================================================
 Target-Wise GES Robotics Anomaly Detection: Linear / Ridge
==============================================================

Purpose
-------
This standalone PCMCI-style script runs the final selected target-wise GES anomaly-detection pipeline with ridge-regularized linear regression. It contains the complete pipeline directly in this file, including data loading, target-wise GES, graph caching, coefficient fitting, anomaly scoring, metrics, plotting, and main().
...
pepper_csv/pepper_normal_ges.npz is reused if present; otherwise the same
target-wise GES learning logic creates it.

Dataset
-------
The full PEPPER robotics dataset is used:
...
parent-lag design matrix X and the current target value is y.
    ...
    model   = Linear / ridge-regularized regression
    formula = beta_hat = argmin_beta || y - X beta ||_2^2 + alpha || beta ||_2^2
    params  = threshold_multiplier = 1.2, ridge_alpha = 10.0
...
class RegressionConfig:
    ...
    def params_text(self) -> str:
        ...
        return f"threshold={self.threshold_multiplier}"


LINEAR_CONFIG = RegressionConfig(
    label="Linear / Ridge",
    regression_kind="linear",
    plot_filename="ges_linear_top_anomalous_variables.png",
...
def read_data(path: str) -> pd.DataFrame:
    """
    Load a PEPPER robotics CSV file and use the timestamp column as the index.

    This keeps the data-loading behavior equivalent to the original GES
    implementation.
    """
...
def ensure_canonical_ges_graph() -> Path:
    ...
    Cache behavior:
    - If pepper_csv/pepper_normal_ges.npz exists, reuse the fixed GES graph.
    - If it is missing, learn the same target-wise GES graph and save it.
    """
    path = graph_cache_path()
    if not path.exists():
        learn_causal_model(str(PREFIX / "normal.csv"), str(path))
    return path
...
def load_robotics_matrices(graph_info):
    """
    Load the full PEPPER robotics normal and attack datasets.

    The full CSV files are used, while the normal training subset is used only
    for offline baseline coefficient fitting.
    """
    normal_df = read_data(str(PREFIX / "normal.csv"))
...
def fit_regression_model(x: np.ndarray, y: np.ndarray, config: RegressionConfig, fitted_transformer=None):
    """
    Fit the Ridge downstream regression model with alpha=10.0.

    Coefficient vectors include the intercept as the first value. The anomaly
    score is based on coefficient drift, so the online refit uses the same
    parent-lag feature layout as the offline baseline fit.
    """
...
def compute_online_errors(data, fine_coeffs, causal_matrix, indices, groups, transformers, config):
    ...
    for t in range(max_time):
        ...
        for col, target_var in enumerate(targets):
            x, y = build_parent_lag_xy(data, groups[target_var], end_idx=end_idx)
            coeffs, _ = fit_regression_model(x, y, config, transformers[target_var])
            delta = coeffs - fine_coeffs[target_var]

            if target_var not in err:
                err[target_var] = np.zeros((max_time, len(delta)))
            err[target_var][t, :] = delta
            norm_agg[t, col] = np.linalg.norm(delta)
...
#                       ANOMALY DETECTION
# ================================================================
def detect_anomalies(err_normal, err_attack, normal_data_len: int, normal: bool, threshold_multiplier: float):
    """
    Count detected anomaly indices from coefficient-drift deviations.
    """
    indices_error = []
    for target_var in err_attack.keys():
        for coef_index in range(err_attack[target_var].shape[1]):
            threshold = threshold_multiplier * np.linalg.norm(
                err_normal[target_var][:normal_data_len, coef_index]
            )
            if normal:
                indices_error += list(
                    np.where(np.abs(err_attack[target_var][normal_data_len:, coef_index]) > threshold)[0]
                )
            else:
                indices_error += list(
                    np.where(np.abs(err_attack[target_var][:, coef_index]) > threshold)[0]
                )
...
def score_and_predict(err_normal, err_eval, normal_data_len: int, normal: bool, threshold_multiplier: float, total_samples=None):
    """
    Return binary predictions and continuous scores from coefficient drift.

    The binary predictions use the exact same threshold rule as
    detect_anomalies(...). The continuous score is the maximum
    threshold-normalized drift over all target/coefficient traces at each time.
    """
    first_key = next(iter(err_eval.keys()))
    if normal:
        n_samples = max(0, err_eval[first_key].shape[0] - normal_data_len)
    else:
        n_samples = err_eval[first_key].shape[0]

    scores = np.zeros(n_samples, dtype=float)
    predictions = np.zeros(n_samples, dtype=bool)

    for target_var in err_eval.keys():
        for coef_index in range(err_eval[target_var].shape[1]):
            threshold = threshold_multiplier * np.linalg.norm(
                err_normal[target_var][:normal_data_len, coef_index]
            )
            threshold_safe = threshold if threshold > 0 else np.finfo(float).eps
            if normal:
                values = np.abs(err_eval[target_var][normal_data_len:, coef_index])
            else:
                values = np.abs(err_eval[target_var][:, coef_index])
            scores = np.maximum(scores, values / threshold_safe)
            predictions |= values > threshold

    if total_samples is not None and n_samples < total_samples:
        pad_width = int(total_samples - n_samples)
        scores = np.pad(scores, (0, pad_width), constant_values=0.0)
        predictions = np.pad(predictions.astype(int), (0, pad_width), constant_values=0).astype(bool)

    return predictions.astype(int), scores

"""
==============================================================
Target-Wise GES Industrial Anomaly Detection: Linear / Ridge
==============================================================

Purpose
-------
This standalone PCMCI-style script runs the target-wise GES anomaly-detection
pipeline with Linear / Ridge regression on the Tennessee Eastman
Process (TEP) industrial dataset.
...
LINEAR_CONFIG = RegressionConfig(
    ...
    plot_filename="linear_ridge_top_anomalous_variables.png",
...
def transform_single_target_inputs(X: np.ndarray, transformer):
    return X
...
def compute_online_errors(data: np.ndarray, fine_coeffs: dict, causal_matrix: np.ndarray, indices: np.ndarray, groups: dict, transformers: dict, config: RegressionConfig):
    ...
    for t in range(max_time):
        ...
        for col, target_var in enumerate(targets):
            X_window, y_window, _ = build_parent_lag_xy(data, groups[target_var], end_idx=end_idx)
            ...
            if target_var not in err:
                err[target_var] = np.zeros((max_time, len(coeffs)))
            err[target_var][t, :] = coeffs - fine_coeffs[target_var]
            norm_agg[t, col] = np.linalg.norm(err[target_var][t, :])
...
def run_linear_experiment(config: RegressionConfig, verbose: bool = True):
    ...
    reporting_helper.generate_plots_and_reports(
        ...
        target_indices=list(groups.keys()),
        directed_edges=[], 

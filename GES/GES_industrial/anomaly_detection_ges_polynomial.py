"""
==============================================================
 Target-Wise GES Industrial Anomaly Detection: Polynomial
==============================================================

Purpose
-------
This standalone PCMCI-style script runs the target-wise GES anomaly-detection
pipeline with Polynomial regression on the Tennessee Eastman Process (TEP)
industrial dataset.
...
# ================================================================
class FittedPolyPipeline:
    def __init__(self, scaler, poly):
        self.scaler = scaler
        self.poly = poly
    def transform(self, X):
        return self.poly.transform(self.scaler.transform(X))
...
def fit_single_target_model(X: np.ndarray, y: np.ndarray, config: RegressionConfig):
    ...
    return coeffs, FittedPolyPipeline(scaler, poly)
...
def compute_online_errors(data: np.ndarray, fine_coeffs: dict, causal_matrix: np.ndarray, indices: np.ndarray, groups: dict, transformers: dict, config: RegressionConfig):
    ...
    for t in range(max_time):
        ...
        for col, target_var in enumerate(targets):
            ...
            transformer = transformers[target_var]
            X_design = transformer.transform(X_window)
